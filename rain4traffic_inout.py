...
# =========================================================
# 4) Build series (inflow / outflow) for plotting
# =========================================================
td = dyna[
    (dyna["date"] == day)
...
# 按小时分别汇总入流/出流
series_in = td.groupby("hour")["inflow"].sum().reindex(hours, fill_value=np.nan)
series_out = td.groupby("hour")["outflow"].sum().reindex(hours, fill_value=np.nan)

y_in = series_in.to_numpy()
y_out = series_out.to_numpy()

rain_levels = [rain_level_by_day[day].get((gid, int(h)), "unknown") for h in hours]


# =========================================================
# 5) Plot: two lines (inflow & outflow); background indicates rain_level
# =========================================================
plt.figure(figsize=(7.2, 4.2))

plt.plot(hours, y_in, marker="o", linewidth=1.6, markersize=3, label="Inflow")
plt.plot(hours, y_out, marker="s", linewidth=1.6, markersize=3, label="Outflow")
...
# --- draw background (用中性灰，避免与两条折线颜色冲突) ---
for s, e, lv in segments:
    ax.axvspan(s, e, facecolor="0.2", alpha=float(alpha_map[lv]), edgecolor="none")
...
# --- legend: first inflow/outflow lines, then rain_level patches ---
handles, labels = ax.get_legend_handles_labels()
for lv in uniq_levels:
    handles.append(Patch(facecolor="0.2", alpha=float(alpha_map[lv]), label=disp_level(lv)))
...
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "rain4traffic_inout.pdf"))
plt.savefig(os.path.join(OUT_DIR, "rain4traffic_inout.svg"))
plt.show()
